...
``\tcode{T2}'' is a reference surrogate type and overload resolution finds
at least one viable dot access function with return type reference-related to ``\tcode{T1}'', or
\item
...
\item
Otherwise,
if ``\tcode{T2}'' is a reference surrogate type, then the best dot access function is selected through
overload resolution as described in\iref{over.match.class.deduct} with ``\cvqual{cv1} \tcode{T1}'' as the target type.
